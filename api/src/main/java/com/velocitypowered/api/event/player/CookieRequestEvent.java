...
/**
 * This event is fired when a cookie from a client is requested either by a
 * proxy plugin or
 * by a backend server. Velocity will wait on this event to finish firing before
 * discarding the
...
public final class CookieRequestEvent implements ResultedEvent<CookieRequestEvent.ForwardResult> {
  ...
  public static final class ForwardResult implements ResultedEvent.Result {
    ...
    /**
     * Prevents the cookie request from being forwarded to the client, the cookie
     * request is
    ...
    /**
     * Allows the cookie request to be forwarded to the client, but silently
     * replaces the
