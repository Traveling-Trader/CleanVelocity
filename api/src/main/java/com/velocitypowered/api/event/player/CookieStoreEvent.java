...
/**
 * This event is fired when a cookie should be stored on a player's client. This
 * process can be
 * initiated either by a proxy plugin or by a backend server. Velocity will wait
 * on this event
 * to finish firing before discarding the cookie (if handled) or forwarding it
 * to the client so
...
public final class CookieStoreEvent implements ResultedEvent<CookieStoreEvent.ForwardResult> {
  ...
  public static final class ForwardResult implements ResultedEvent.Result {
    ...
    /**
     * Prevents the cookie from being forwarded to the client, the cookie is handled
     * by the proxy.
     *
    ...
    /**
     * Allows the cookie to be forwarded to the client so that it can store it, but
     * silently
