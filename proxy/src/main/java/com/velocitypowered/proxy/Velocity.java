...
/**
 * The main class. Responsible for parsing command line arguments and then
 * launching the
 * proxy.
 */
public class Velocity {
  ...
  static {
    ...
    // We use BufferedImage for favicons, and on macOS this puts the Java
    // application in the dock.
    // How inconvenient. Force AWT to work with its head chopped off.
    System.setProperty("java.awt.headless", "true");

    // If Velocity's natives are being extracted to a different temporary directory,
    // make sure the
    ...
    // Disable the resource leak detector by default as it reduces performance.
    // Allow the user to
  ...
  /**
   * Main method that the JVM will call when {@code java -jar velocity.jar} is
   * executed.
   *
  ...
  public static void main(String... args) {
    ...
    logger.info("Enabled in {}s!", new DecimalFormat("#.##").format(bootTime));
    server.getConsoleCommandSource().start();

    // If we don't have a console available (because SimpleTerminalConsole
    // returned), then we still
    // need to wait, otherwise the JVM will reap us as no non-daemon threads will be
    // active once the
    // main thread exits.
