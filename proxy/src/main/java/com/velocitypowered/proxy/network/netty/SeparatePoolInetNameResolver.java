...
/**
 * An implementation of {@code InetNameResolver} that performs blocking DNS name
 * lookups
 * in a separate thread, avoiding blocking the Netty threads for an extended
 * period of time
    ...
    try {
      promise.addListener(future -> {
        if (future.isSuccess()) {
          ...
          List<InetAddress> result = (List<InetAddress>) future.getNow();
