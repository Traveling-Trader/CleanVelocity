...
/**
 * Manages endpoints managed by Velocity, along with initializing the Netty
 * event loop group.
 */
public final class ConnectionManager {
  ...
  // These are intentionally made public for plugins like ViaVersion, which inject
  // their own
  ...
  public void bind(final InetSocketAddress address) {
    ...
    final int binds = server.getConfiguration().isEnableReusePort()
        ? ((MultithreadEventExecutorGroup) this.workerGroup).executorCount()
        : 1;

    for (int bind = 0; bind < binds; bind++) {
      // Wait for each bind to open. If we encounter any errors, don't try to bind
      // again.
      int finalBind = bind;
   ...
   *
   * @param group the event loop group to use. Use {@code null} for the default
   *              worker group.
  ...
  public void close(InetSocketAddress oldBind) {
    ...
    // Fire proxy close event to notify plugins of socket close. We block since
    // plugins
    // should have a chance to be notified before the server stops accepting
    // connections.
