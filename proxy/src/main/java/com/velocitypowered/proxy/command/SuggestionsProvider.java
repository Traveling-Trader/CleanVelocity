...
/**
 * Provides suggestions for a given command input. Similar to
 * {@link CommandDispatcher#getCompletionSuggestions(ParseResults)}, avoids
 * fully parsing the input and performs one requirement check per considered
 * node.
 *
...
final class SuggestionsProvider<S> {
  ...
  /**
   * Returns whether a literal node with the given lowercase name should be
   * considered for
  ...
  /**
   * Merges the suggestions provided by the {@link Command} associated to the
   * given alias node and the hints given during registration for the given
   * input. The context is not mutated by this method. The reader's cursor may
   * be modified.
   *
  ...
  /**
   * Returns the suggestions provided by the {@link Command} associated to the
   * specified arguments node for the given input. The reader and context are
  ...
  /**
   * Returns the suggestions provided by the matched hint nodes for the given
   * input. The reader and context are not mutated by this method.
   *
   * @param alias   the alias node
   * @param reader  the input reader
  ...
  /**
   * Parses the hint nodes under the given node, which is either an alias node
   * of a {@link Command} or another hint node. The reader and context are not
   ...
   * @see VelocityCommandMeta#copyHints(CommandMeta) for the conditions under
   *      which the returned
  ...
  /**
   * Returns a future that is completed with the result of merging the
   * {@link Suggestions} the given
   * futures complete with. The results of the futures that complete exceptionally
   * are ignored.
   *
