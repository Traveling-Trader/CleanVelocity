...
public class VelocityCommandManager implements CommandManager {
   ...
   * {@linkplain CommandRegistrar#registrableSuperInterface() registrable
   * superinterface} of the
   * given registrar.
   *
  ...
  public CompletableFuture<Boolean> executeImmediatelyAsync(
    ...
    return CompletableFuture.supplyAsync(
        () -> this.parse(cmdLine, source), figureAsyncExecutorForParsing()).thenCompose(
  ...
  private Executor figureAsyncExecutorForParsing() {
    ...
    if (thread instanceof FastThreadLocalThread) {
      ...
      // it's some other thread that isn't a Netty event loop thread. direct execution
      // it is!
