...
public class MinecraftVarintFrameDecoder extends ByteToMessageDecoder {
  ...
  private static final QuietDecoderException INVALID_PREAMBLE = new QuietDecoderException("Invalid packet preamble");
  ...
  protected void decode(ChannelHandlerContext ctx, ByteBuf in, List<Object> out)
    ...
    if (packetStart == -1) {
      in.clear();
      // Apply a more strict check in serverbound direction, we really shouldn't be
      // seeing this many 0x00s
      // even from the server, the only reason we even allow these is due to bugged
      // servers
    ...
    try {
      ...
      // Reset buffer to consistent state before propagating exception to prevent
      // memory leaks
      in.resetReaderIndex();
      throw e;
    }
  ...
  private static int readRawVarInt21(ByteBuf buffer) {
    if (buffer.readableBytes() < 4) {
      // we don't have enough that we can read a potentially full varint, so fall back
      // to
      // the slow path.
    ...
    if (atStop == 0) {
      // all bytes have the high bit set, so the varint we are trying to decode is too
      // wide
    ...
    //
    // > The idea is that thisVarintMask has 0s above the first one of
    // firstOneOnStop, and 1s at
    // > and below it. For example if firstOneOnStop is 0x800080 (where the last
    // 0x80 is the only
    // > one that matters), then thisVarintMask is 0xFF.
    //
    // this is also documented in Hacker's Delight, section 2-1 "Manipulating
    // Rightmost Bits"
    int preservedBytes = wholeOrMore & (atStop ^ (atStop - 1));

    // merge together using this trick:
