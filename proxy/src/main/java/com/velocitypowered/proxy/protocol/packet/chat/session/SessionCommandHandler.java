...
public class SessionCommandHandler extends RateLimitedCommandHandler<SessionPlayerCommandPacket> {
  ...
  private MinecraftPacket consumeCommand(SessionPlayerCommandPacket packet) {
    ...
    if (packet.isSigned()) {
      // Any signed message produced by the client *must* be passed through to the
      // server in order to maintain a
    ...
    // An unsigned command with a 'last seen' update will not happen as of 1.20.5+,
    // but for earlier versions - we still
