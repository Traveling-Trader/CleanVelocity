...
public final class PluginMessageUtil {
  ...
  /**
   * Determines whether or not this is a brand plugin message. This is shown on
   * the client.
   *
  ...
  /**
   * Determines whether or not this plugin message is being used to register
   * plugin channels.
   *
   * @param message the plugin message
  ...
  /**
   * Determines whether or not this plugin message is being used to unregister
   * plugin channels.
   *
   * @param message the plugin message
  ...
  public static List<ChannelIdentifier> getChannels(int existingChannels,
    ...
    if (!message.content().isReadable()) {
      // If we try to split this, we will get an one-element array with the empty
      // string, which
    ...
    checkArgument(existingChannels + channels.length <= ConnectedPlayer.MAX_CLIENTSIDE_PLUGIN_CHANNELS,
        "too many channels: %s + %s > %s", existingChannels, channels.length,
        ConnectedPlayer.MAX_CLIENTSIDE_PLUGIN_CHANNELS);
    ImmutableList.Builder<ChannelIdentifier> channelIdentifiers = ImmutableList
        .builderWithExpectedSize(channels.length);
    try {
  ...
  /**
   * Rewrites the brand message to use the configured proxy brand.
   *
   * @param message         the plugin message
   * @param proxyBrand      the configured brand name
  ...
  public static PluginMessagePacket rewriteMinecraftBrand(PluginMessagePacket message,
      ...
      ProtocolVersion protocolVersion) {
    checkNotNull(message, "message");
    checkNotNull(proxyBrand, "proxyBrand");
    ...
    if (protocolVersion.noLessThan(ProtocolVersion.MINECRAFT_1_8)) {
      ProtocolUtils.writeString(rewrittenBuf, proxyBrand);
    } else {
      rewrittenBuf.writeCharSequence(proxyBrand, StandardCharsets.UTF_8);
    }
   ...
   * brand messages. In
   * order to accommodate their broken behavior, we'll first try to read in the
   * 1.8 format, and if
   * that fails, treat it as a 1.7-format message (which has no prefixed length).
   * (The message
  ...
  /**
   * Transform a plugin message channel from a "legacy" (less than 1.13) form to a
   * modern one.
   *
  ...
  public static String transformLegacyToModernChannel(String name) {
    ...
    return switch (name) {
      ...
      default -> {
        ...
        // scheme as
        // BungeeCord does to transform channels, but also removes clearly invalid
        // characters as
        // well.
