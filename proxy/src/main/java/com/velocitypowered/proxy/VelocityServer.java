...
public class VelocityServer implements ProxyServer, ForwardingAudience {
  ...
  void start() {
    logger.info("Enabling {} {}.", getVersion().getName(), getVersion().getVersion());
    console.setupStreams();
    pluginManager.registerPlugin(this.createVirtualPlugin());

    // Yes, you're reading that correctly. We're generating a 1024-bit RSA keypair.
    // Sounds
    // dangerous, right? We're well within the realm of factoring such a key...
    //
    // You can blame Mojang. For the record, we also don't consider the Minecraft
    // protocol
    // encryption scheme to be secure, and it has reached the point where any
    // serious cryptographic
    // protocol needs a refresh. There are multiple obvious weaknesses, and this is
    // far from the
    // most serious.
    //
    // If you are using Minecraft in a security-sensitive application, *I don't know
    // what to say.*
    ...
    // Go ahead and fire the proxy initialization event. We block since plugins
  ...
  private void loadPlugins() {
    ...
    // Register the plugin main classes so that we can fire the proxy initialize
    // event
   ...
   *
   * @return {@code true} if successful, {@code false} if we can't read the
   *         configuration
  ...
  public boolean reloadConfiguration() throws IOException {
    ...
    // Re-register servers. If a server is being replaced, make sure to note what
    // players need to
    ...
    // If we had any players to evacuate, let's move them now. Wait until they are
    // all moved off.
    if (!evacuate.isEmpty()) {
  ...
  public void shutdown(boolean explicitExit, Component reason) {
    ...
    Runnable shutdownProcess = () -> {
      ...
      // Since we manually removed the shutdown hook, we need to handle the shutdown
      // ourselves.
      LogManager.shutdown();
  ...
  /**
   * Calls {@link #shutdown(boolean, Component)} with the default reason "Proxy
   * shutting down."
   *
   * @param explicitExit whether the user explicitly shut down the proxy
   */
