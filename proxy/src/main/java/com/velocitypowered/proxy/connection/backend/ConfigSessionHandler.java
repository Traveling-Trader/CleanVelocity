...
/**
 * A special session handler that catches "last minute" disconnects. This
 * version is to accommodate
...
public class ConfigSessionHandler implements MinecraftSessionHandler {
      ...
      if (serverResourcePackSendEvent.getResult().isAllowed()) {
        ...
        if (serverConn.getPlayer().resourcePackHandler().hasPackAppliedByHash(toSend.getHash())) {
          ...
          if (serverConn.getConnection() != null) {
            // We can technically skip these first 2 states, however, for conformity to
            // normal state flow expectations...
  ...
  public boolean handle(FinishedUpdatePacket packet) {
    ...
    final ConnectedPlayer player = serverConn.getPlayer();
    final ClientConfigSessionHandler configHandler = (ClientConfigSessionHandler) player.getConnection()
  ...
  public boolean handle(DisconnectPacket packet) {
    serverConn.disconnect();
    // If the player receives a DisconnectPacket without a connection to a server in
    // progress,
  ...
  public boolean handle(PluginMessagePacket packet) {
    if (PluginMessageUtil.isMcBrand(packet)) {
      serverConn.getPlayer().getConnection().write(
          PluginMessageUtil.rewriteMinecraftBrand(packet, server.getConfiguration().getBrand(),
