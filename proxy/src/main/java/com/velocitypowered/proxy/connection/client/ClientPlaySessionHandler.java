...
/**
 * Handles communication with the connected Minecraft client. This is
 * effectively the primary nerve
...
public class ClientPlaySessionHandler implements MinecraftSessionHandler {
  ...
  public void activated() {
    ...
    Collection<ChannelIdentifier> channels = server.getChannelRegistrar()
  ...
  public boolean handle(PluginMessagePacket packet) {
    ...
    if (serverConn != null && backendConn != null) {
      if (backendConn.getState() != StateRegistry.PLAY) {
        ...
        List<ChannelIdentifier> channels = PluginMessageUtil.getChannels(this.player.getClientsideChannels().size(),
            packet,
        ...
        List<ChannelIdentifier> channels = PluginMessageUtil.getChannels(0, packet, this.player.getProtocolVersion());
        ...
        if (!player.getPhase().handle(player, packet, serverConn)) {
          ...
          if (id == null) {
            ...
            if (!player.getPhase().consideredComplete() || !serverConn.getPhase()
                .consideredComplete()) {
              // The client is trying to send messages too early. This is primarily caused by
              // mods,
              // but further aggravated by Velocity. To work around these issues, we will
              // queue any
              // non-FML handshake messages to be sent once the FML handshake has completed or
              // the
            ...
            server.getEventManager().fire(event).thenAcceptAsync(pme -> {
              if (pme.getResult().isAllowed()) {
                ...
                if (!player.getPhase().consideredComplete() || !serverConn.getPhase()
                    .consideredComplete()) {
                  // We're still processing the connection (see above), enqueue the packet for
                  // now.
  ...
  public boolean handle(JoinGamePacket packet) {
    // Forward the packet as normal, but discard any chat state we have queued - the
    // client will do this too
    player.discardChatQueue();
    return false;
  }
  ...
  public void writabilityChanged() {
    ...
    if (!writable) {
      ...
      // memory. Make
      // sure to do it on a future invocation of the event loop, otherwise while the
      // issue will
      // fix itself, we'll still disable auto-reading and instead of backpressure
      // resolution, we
      // get client timeouts.
  ...
  /**
   * Handles the {@code JoinGame} packet. This function is responsible for
  ...
  public void handleBackendJoinGame(JoinGamePacket joinGame, VelocityServerConnection destination) {
    ...
    if (!spawned) {
      ...
      // The player is switching from a server already, so we need to tell the client
      // to change
    ...
    if (player.getProtocolVersion().noLessThan(ProtocolVersion.MINECRAFT_1_20_2)) {
      ...
      // Remove previous boss bars. These don't get cleared when sending JoinGame (up
      // until 1.20.2),
  ...
  private void doFastClientServerSwitch(JoinGamePacket joinGame) {
    // In order to handle switching to another server, you will need to send two
    // packets:
    //
    // - The join game packet from the backend server, with a different dimension
    // - A respawn with the correct dimension
    //
    // Most notably, by having the client accept the join game packet, we can work
    // around the need
    // to perform entity ID rewrites, eliminating potential issues from rewriting
    // packets and
    ...
    if (player.getProtocolVersion().lessThan(ProtocolVersion.MINECRAFT_1_16)) {
      // Before Minecraft 1.16, we could not switch to the same dimension without
      // sending an
      // additional respawn. On older versions of Minecraft this forces the client to
      // perform
  ...
  private void doSafeClientServerSwitch(JoinGamePacket joinGame) {
    // Some clients do not behave well with the "fast" respawn sequence. In this
    // case we will use
    // a "safe" respawn sequence that involves sending three packets to the client.
    // They have the
    // same effect but tend to work better with buggier clients (Forge 1.8 in
    // particular).
  ...
  private boolean handleCommandTabComplete(TabCompleteRequestPacket packet) {
    ...
    if (!server.getCommandManager().hasCommand(commandLabel, player)) {
      if (player.getProtocolVersion().lessThan(ProtocolVersion.MINECRAFT_1_13)) {
        // Outstanding tab completes are recorded for use with 1.12 clients and below to
        // provide
