...
public class BackendPlaySessionHandler implements MinecraftSessionHandler {
  ...
  public boolean handle(StartUpdatePacket packet) {
    ...
    // Even when not auto reading messages are still decoded. Decode them with the
    // correct state
    ...
    if (PluginMessageUtil.isMcBrand(packet)) {
      PluginMessagePacket rewritten = PluginMessageUtil
          .rewriteMinecraftBrand(packet,
              server.getConfiguration().getBrand(), playerConnection.getProtocolVersion());
  ...
  public boolean handle(AvailableCommandsPacket commands) {
    ...
    // In 1.21.6 a confirmation prompt was added when executing a command via
    // `run_command` click
    // action if the command is unknown. To prevent this prompt we have to send the
    // command.
  ...
  public boolean handle(ServerDataPacket packet) {
    server.getServerListPingHandler().getInitialPing(this.serverConn.getPlayer()).thenComposeAsync(
        ...
        playerConnection.eventLoop()).thenAcceptAsync(
            pingEvent -> this.playerConnection.write(
