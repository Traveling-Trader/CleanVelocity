...
public class MinecraftConnection extends ChannelInboundHandlerAdapter {
   ...
   * @return A {@link ChannelFuture} that will complete when packet is
   *         successfully sent
   */
  @Nullable
  ...
  /**
   * Determines whether or not the channel should continue reading data
   * automatically.
   *
  ...
  public void setAutoReading(boolean autoReading) {
    ...
    if (autoReading) {
      // For some reason, the channel may not completely read its queued contents once
      // autoread
      // is turned back on, even though toggling autoreading on should handle things
      // automatically.
  ...
  /**
   * Sets the compression threshold on the connection. You are responsible for
   * sending {@link
  ...
  public void setCompressionThreshold(int threshold) {
    ...
    if (threshold == -1) {
      ...
      MinecraftCompressorAndLengthEncoder encoder = (MinecraftCompressorAndLengthEncoder) channel.pipeline()
