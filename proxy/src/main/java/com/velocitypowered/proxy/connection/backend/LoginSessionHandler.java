...
public class LoginSessionHandler implements MinecraftSessionHandler {
  ...
  public boolean handle(ServerLoginSuccessPacket packet) {
    if (server.getConfiguration().getPlayerInfoForwardingMode() == PlayerInfoForwarding.MODERN
      ...
      resultFuture
          .complete(ConnectionRequestResults.forDisconnect(MODERN_IP_FORWARDING_FAILURE, serverConn.getServer()));
      serverConn.disconnect();
      return true;
    }

    // The player has been logged on to the backend server, but we're not done yet.
    // There could be
    // other problems that could arise before we get a JoinGame packet from the
    // server.
    ...
    if (smc.getProtocolVersion().lessThan(ProtocolVersion.MINECRAFT_1_20_2)) {
      ...
      if (player.getConnection()
          .getActiveSessionHandler() instanceof ClientPlaySessionHandler clientPlaySessionHandler) {
