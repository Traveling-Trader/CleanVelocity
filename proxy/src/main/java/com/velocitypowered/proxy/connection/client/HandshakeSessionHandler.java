...
/**
 * The initial handler used when a connection is established to the proxy. This
 * will either
 * transition to {@link StatusSessionHandler} or
 * {@link InitialLoginSessionHandler} as soon as the
...
public class HandshakeSessionHandler implements MinecraftSessionHandler {
  ...
  public boolean handle(LegacyPingPacket packet) {
    ...
    final StatusSessionHandler handler = new StatusSessionHandler(server,
  ...
  private void handleLogin(HandshakePacket handshake, InitialInboundConnection ic) {
    if (!handshake.getProtocolVersion().isSupported()) {
      // Bump connection into correct protocol state so that we can send the
      // disconnect packet.
    ...
    // If the proxy is configured for modern forwarding, we must deny connections
    // from 1.12.2
  ...
  private ConnectionType getHandshakeConnectionType(HandshakePacket handshake) {
    ...
    if (handshake.getServerAddress().endsWith(LegacyForgeConstants.HANDSHAKE_HOSTNAME_TOKEN)
      ...
      // 1.7 Forge will not notify us during handshake. UNDETERMINED will listen for
      // incoming
      // forge handshake attempts. Also sends a reset handshake packet on every
      // transition.
      return ConnectionTypes.UNDETERMINED_17;
    } else {
      // Note for future implementation: Forge 1.13+ identifies itself using a
      // slightly different
      // hostname token.
  ...
  static String cleanVhost(String hostname) {
    // Clean out any anything after any zero bytes (this includes BungeeCord
    // forwarding and the
    ...
    // If we connect through an SRV record, there will be a period at the end (DNS
    // usually elides
    // this ending octet).
  ...
  public String toString() {
    return "[initial connection] " + this.connection.getRemoteAddress();
  }
  ...
  private record LegacyInboundConnection(
    ...
    @Override
    public String toString() {
      return "[legacy connection] " + this.getRemoteAddress();
    }
